...
using ProcureFlow.Modules.Vendors.PublicApi;
    ...
    ICurrentTenant currentTenant) : ICommandHandler<SubmitPoCommand, Result<PurchaseOrderResponse>>
{
    public async Task<Result<PurchaseOrderResponse>> HandleAsync(SubmitPoCommand request, CancellationToken ct)
    {
        ...
        FeasibilityResult feasibility = feasibilityEngine.Evaluate(new FeasibilityInput(
            vendorEligible, 50m, budgetHeadroom, dutyExposure, 30, true, po.TotalAmount));

        var snapshot = new FeasibilitySnapshot(feasibility.Score, feasibility.Verdict.ToString(),
            feasibility.Reasons, DateTime.UtcNow);
