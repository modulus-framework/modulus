...
internal static class SampleDataSeeder
{
    ...
    private static async Task SeedSettingsAsync(IServiceScope scope, ILogger logger)
    {
        ...
        var settings = new[]
        {
            ("app.name", "ProcureFlow", "General", "Application display name", true),
            ("app.default-locale", "en-US", "General", "Default UI locale", true),
            ("company.support-email", "support@procureflow.com", "Company", "Public support contact", true),
    ...
    private static async Task SeedTenantsAsync(IServiceScope scope, ILogger logger)
    {
        ...
        var connectionString = "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow";
    ...
    private static async Task SeedNotificationsAsync(IServiceScope scope, ILogger logger)
    {
        ...
        {
            ("Welcome to ProcureFlow", "Your workspace is ready. Explore the Settings, Features, Tenants and Media modules.", NotificationType.Success),
