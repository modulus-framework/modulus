...
All modules are auto-discovered via `AddModulus<ProcureFlowHostModule>()` and share:
...
src/
├── API/
│   └── ProcureFlow.Api/          # Composition root, middleware, startup
├── Shared/                        # Shared kernel (framework-level utilities)
│   ├── ProcureFlow.Shared.Domain/
│   ├── ProcureFlow.Shared.Application/
│   ├── ProcureFlow.Shared.Infrastructure/
│   └── ProcureFlow.Shared.Presentation/
└── Modules/
...
```bash
# Development (SQLite)
dotnet restore ProcureFlow.slnx
dotnet build ProcureFlow.slnx
dotnet run --project src/API/ProcureFlow.Api -- --seed   # migrate + seed sample data
dotnet run --project src/API/ProcureFlow.Api             # runs on http://localhost:5000
```
...
{
  "ConnectionStrings": {
    "Vendors": "Host=localhost;Port=5432;Database=ProcureFlow;Username=procureflow;Password=...",
    "Budgeting": "Host=localhost;Port=5432;Database=ProcureFlow;Username=procureflow;Password=...",
    // ... all other modules
  }
}
```
...
Per-module connection strings are configured in `appsettings.json`. All modules use the same `ProcureFlow` database — they keep their tables isolated via per-module PostgreSQL schemas in production, or separate tables in SQLite for development.

```json
{
  "ConnectionStrings": {
    "Vendors": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "Budgeting": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "Customs": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "Procurement": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "TradeFinance": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "Import": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "Inventory": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow",
    "Costing": "Host=localhost;Port=5432;Database=ProcureFlow;Username=ProcureFlow;Password=ProcureFlow"
  }
}
```
...
dotnet test ProcureFlow.slnx --filter "Category=Unit"

# Run domain tests
dotnet test tests/ProcureFlow.DomainTests/ProcureFlow.DomainTests.csproj

# Run integration tests (requires SAC bypass)
dotnet test tests/ProcureFlow.IntegrationTests/ProcureFlow.IntegrationTests.csproj
```
...
4. **Extend permissions** in `ProcureFlowHostModule.cs` for your specific domain requirements
