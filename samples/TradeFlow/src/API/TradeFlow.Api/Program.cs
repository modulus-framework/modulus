...
using TradeFlow.Modules.Notifications.Presentation;
...
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("TradeFlow.Modules.Identity.UnitTests")]
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("TradeFlow.E2ETests")]
...
[
    TradeFlow.Modules.Identity.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Tenants.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Configuration.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Notifications.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Vendors.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.OrgStructure.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Budgeting.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Customs.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Procurement.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.TradeFinance.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Import.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Inventory.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Costing.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.Finance.Application.AssemblyReference.Assembly,
    TradeFlow.Modules.VirtualFileExplorer.Application.AssemblyReference.Assembly,
];
...
// the integration-event registry, and — via its own interface-based assembly scan —
// every IDomainEventHandler<>/IIntegrationEventHandler<> in the module Application
// assemblies (the single source of handler registration; modules no longer roll
// their own reflection loops). The Domain assemblies are passed too so the registry
// knows every event type that now implements IIntegrationEvent (the transactional
// outbox resolves rows by the routing names those types declare).
Assembly[] moduleDomainAssemblies =
[
    typeof(TradeFlow.Modules.Identity.Domain.Events.UserCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Tenants.Domain.Events.TenantCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Configuration.Domain.Events.FeatureFlagCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Notifications.Domain.Events.NotificationCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Vendors.Domain.Events.VendorCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.OrgStructure.Domain.Events.OrgNodeCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Budgeting.Domain.Events.BudgetCreatedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Customs.Domain.Events.BoeSubmittedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Procurement.Domain.Events.PrApprovedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.TradeFinance.Domain.Events.LcIssuedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Import.Domain.Events.ImportFileClosedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Inventory.Domain.Events.GrnPostedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Costing.Domain.Events.CostSheetFinalizedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.Finance.Domain.Events.JournalPostedDomainEvent).Assembly,
    typeof(TradeFlow.Modules.VirtualFileExplorer.Domain.Events.VirtualFolderCreatedDomainEvent).Assembly,
];
...
// The authorization store is EF-backed but deliberately outside the module migration
// loop and has no migrations (see MigrateAuthorizationStoreAsync). It needs its OWN
// database so EnsureCreated is not a no-op on the shared TradeFlow DB.
string authorizationConnectionString = builder.Configuration.GetConnectionString("Authorization")!;
...
// Turn on delegation + temporary access: registers the DelegationResolver (over the
// EF-backed store above) and decorates IPermissionResolver with
// DelegationAwarePermissionResolver, so a delegation created via the management API
// takes effect at ICurrentUser.HasPermission immediately — time-bounded, revocable,
// and capped by the delegator's own direct authority.
builder.Services.AddDelegation();
...
// 7.5. Multi-tenancy: resolve the tenant from the X-Tenant-Id header, and when
// no tenant can be resolved (no header / unknown id) fall back to the explicit
// host scope. The framework's tenant query filter is fail-closed (an unresolved
// tenant sees nothing); this sample seeds its data under TenantId = Guid.Empty
// (the "no tenant header" scope), so the host-scope fallback is what keeps the
// no-header API flow able to read the seeded rows.
app.UseMultiTenancy();
app.Use(async (context, next) =>
{
    var tenant = context.RequestServices.GetRequiredService<ICurrentTenant>();
    if (!tenant.IsAvailable && !tenant.IsHost)
    {
        using var _ = tenant.Change(null);
        await next(context);
    }
    else
    {
...
app.MapModulusEndpoints(
    TradeFlow.Modules.Identity.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Tenants.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Configuration.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Notifications.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Vendors.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.OrgStructure.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Budgeting.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Customs.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Procurement.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.TradeFinance.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Import.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Inventory.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Costing.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.Finance.Presentation.AssemblyReference.Assembly,
    TradeFlow.Modules.VirtualFileExplorer.Presentation.AssemblyReference.Assembly);
