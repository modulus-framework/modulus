...
using TradeFlow.Modules.WorkflowEngine.Application.Commands;
...
using TradeFlow.Modules.WorkflowEngine.Domain.Errors;
using TradeFlow.Modules.WorkflowEngine.Domain.Repositories;
using TradeFlow.Modules.WorkflowEngine.Domain.Services;
using TradeFlow.Shared.Domain;

namespace TradeFlow.Modules.WorkflowEngine.Application.Commands;
    ...
    ICurrentUser currentUser) : ICommandHandler<StartWorkflowInstanceCommand, Result<WorkflowInstanceResponse>>
{
    public async Task<Result<WorkflowInstanceResponse>> HandleAsync(StartWorkflowInstanceCommand request, CancellationToken ct)
    {
        ...
        var steps = WorkflowStepParser.Parse(definition.StepsJson);
        foreach (var step in steps)
        {
            // Evaluate "when" condition — skip steps whose condition is false
            bool? whenMet = ConditionEvaluator.Evaluate(step.When, request.ContextJson);
            if (whenMet == false)
                continue;

            // Resolve step type
            string stepType = step.Type.ToLowerInvariant() switch
            {
                "approval" or "approval-chain" => StepType.Approval.ToString(),
                "gate" => StepType.Condition.ToString(),
                "system" => StepType.SystemAction.ToString(),
                "parallel" => StepType.Parallel.ToString(),
                "timer" => StepType.Timer.ToString(),
                "notification" => StepType.Notification.ToString(),
                _ => StepType.Approval.ToString()
            };

            var task = new WorkflowTask(
                instance.Id, instance.Id, step.Id, stepType,
                null, step.AssigneePosition, null,
    ...
    ICurrentUser currentUser) : ICommandHandler<CompleteTaskCommand, Result<WorkflowInstanceResponse>>
{
    public async Task<Result<WorkflowInstanceResponse>> HandleAsync(CompleteTaskCommand request, CancellationToken ct)
    {
        ...
        // SoD: creator cannot approve their own task (BR-WF-01)
        if (decision == TaskDecision.Approve && task.AssigneeUserId == userId)
            return Result.Failure<WorkflowInstanceResponse>(WorkflowErrors.SelfApprovalForbidden());
...
public sealed class ReassignTaskHandler(
    IWorkflowTaskRepository taskRepository,
    IUnitOfWork unitOfWork) : ICommandHandler<ReassignTaskCommand, Result<WorkflowTaskResponse>>
{
