...
public sealed class BillOfEntry : AggregateRoot
{
    ...
    public void Assess(decimal tolerancePct, IEnumerable<AssessedDutyLine> dutyLines, decimal customsExchangeRate)
    {
        Transition(BoeStatus.Assessed);
        TolerancePct = tolerancePct;

        decimal assessedTti = _lines.Sum(l => l.AssessedTtiBdt ?? 0m);
        Raise(new BoeAssessedDomainEvent(Id, TenantId, FileId, BoeNo, assessedTti,
            dutyLines.ToList().AsReadOnly(), customsExchangeRate));
    }
