...
public enum CertificateOfOriginStatus
{
    Draft = 1,
    Verified = 2,
    Mismatch = 3,
...
public sealed class CertificateOfOrigin : AggregateRoot
{
    private CertificateOfOrigin() { }

    private CertificateOfOrigin(Guid id, Guid tenantId, Guid fileId, Guid? ciId,
        CertificateOfOriginType type, string originCountry, string documentNo,
        string? issuerName, DateOnly issuedOn, DateOnly? expiryDate)
    {
        Id = id;
        TenantId = tenantId;
        FileId = fileId;
        ...
        Status = CertificateOfOriginStatus.Draft;
        PreferentialEligible = type != CertificateOfOriginType.Generic;
    }

    public Guid TenantId { get; private set; }
    public Guid FileId { get; private set; }
    public Guid? CiId { get; private set; }
    public CertificateOfOriginType Type { get; private set; }
    public string OriginCountry { get; private set; } = null!;
    public string DocumentNo { get; private set; } = null!;
    public string? IssuerName { get; private set; }
    public DateOnly IssuedOn { get; private set; }
    public DateOnly? ExpiryDate { get; private set; }
    public CertificateOfOriginStatus Status { get; private set; }
    public bool PreferentialEligible { get; private set; }
    public string? MismatchReason { get; private set; }

    public static CertificateOfOrigin Create(Guid tenantId, Guid fileId, Guid? ciId,
        CertificateOfOriginType type, string originCountry, string documentNo,
        string? issuerName, DateOnly issuedOn, DateOnly? expiryDate)
    {
        if (string.IsNullOrWhiteSpace(originCountry))
            throw new ArgumentException("Origin country is required", nameof(originCountry));
        if (string.IsNullOrWhiteSpace(documentNo))
            throw new ArgumentException("Document number is required", nameof(documentNo));

        return new CertificateOfOrigin(Guid.NewGuid(), tenantId, fileId, ciId,
            type, originCountry.Trim(), documentNo.Trim(),
            issuerName?.Trim(), issuedOn, expiryDate);
    }

    public Result CheckOriginMismatch(string ciOriginCountry)
    {
        if (!string.Equals(OriginCountry, ciOriginCountry, StringComparison.OrdinalIgnoreCase))
        {
            Status = CertificateOfOriginStatus.Mismatch;
            MismatchReason = $"COO origin '{OriginCountry}' does not match CI origin '{ciOriginCountry}'";
            return Result.Failure(Error.BusinessRule("Coo.OriginMismatch", MismatchReason));
        }
        Status = CertificateOfOriginStatus.Verified;
        return Result.Success();
    }
}
