...
using TradeFlow.Modules.Inventory.Application.Queries;
using TradeFlow.Modules.Inventory.Domain.Entities;
using TradeFlow.Shared.Domain;

namespace TradeFlow.Modules.Inventory.Presentation;
...
internal sealed class IssueStockEndpoint : Endpoint<IssueStockEndpoint.Request, StockItemResponse>
{
    private readonly IMediator _mediator;

    public IssueStockEndpoint(IMediator mediator) => _mediator = mediator;

    public override void Configure()
    {
        Post("/stock-items/{siteId}/{itemId}/issue");
        Tag(Tags.Stock);
        Summary("Issue stock against FEFO batches (BR-VAL-05)");
    }

    public override async Task HandleAsync(Request req, CancellationToken ct)
    {
        Result<StockItemResponse> result = await _mediator.SendAsync(
            new IssueStockCommand(req.SiteId, req.ItemId, req.Quantity, req.SourceDoc, req.Reference), ct);
        await EndpointHelper.ResolveAsync(HttpContext, result, ct);
    }
...
internal sealed class CreateManualAdjustmentEndpoint : Endpoint<CreateManualAdjustmentEndpoint.Request, StockItemResponse>
{
    private readonly IMediator _mediator;

    public CreateManualAdjustmentEndpoint(IMediator mediator) => _mediator = mediator;

    public override void Configure()
    {
        Post("/stock-items/{siteId}/{itemId}/adjust");
        Tag(Tags.Stock);
        Summary("Manual stock adjustment with reason taxonomy (BR-VAL-03)");
    }

    public override async Task HandleAsync(Request req, CancellationToken ct)
    {
        Result<StockItemResponse> result = await _mediator.SendAsync(
            new CreateManualAdjustmentCommand(req.SiteId, req.ItemId, req.QuantityDelta,
                req.UnitCost, req.ReasonTaxonomy, req.SourceDoc, req.Reference), ct);
        await EndpointHelper.ResolveAsync(HttpContext, result, ct);
    }
    ...
    internal sealed class Request
    {
        ...
        public decimal UnitCost { get; set; }
        public string ReasonTaxonomy { get; set; } = string.Empty;
        public string SourceDoc { get; set; } = string.Empty;
        public string Reference { get; set; } = string.Empty;
    }
}