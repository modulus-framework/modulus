# TradeFlow Phase-1 MVP — Implementation Plan

Status: **Approved** — plan of record for delivering the TradeFlow BRS (v2.0) Phase-1 MVP on top of ModulusSampleErp.
...
Source documents: `samples/TradeFlow/BRS-Core.md`, `BRS-Business-Rules.md` (~150 phased rules), `BRS-Phasing-Implementation.md`. Rule IDs below (`BR-*`) refer to the business-rules register; every P1 rule must map to ≥1 test carrying the rule ID in its name.
...
3. Shared kernel: typed ids + engine seams above; extend `Shared.Domain.AppPermissions` with TradeFlow capability groups.
