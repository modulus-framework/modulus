...
- Single realm `procureflow` (operationally simpler than realm-per-tenant at 500 tenants); tenants modeled as Keycloak **organizations/groups**; JWT carries `tenant_id`, `company_ids`, `roles`, `plan`.
