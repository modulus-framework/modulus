...
/// <c>modulus migrate</c>) to construct <see cref="NotificationDbContext"/>
/// without the application's DI container or an HTTP request. At runtime the app
/// still builds the context through DI (see <see cref="NotificationModule"/>);
/// this type is only used when scaffolding or applying migrations.
/// </summary>
/// <remarks>
/// The connection string is read from the <c>NOTIFICATION_CONNECTION</c>
    ...
    : IDesignTimeDbContextFactory<NotificationDbContext>
{
    public NotificationDbContext CreateDbContext(string[] args)
    {
        var connectionString =
            Environment.GetEnvironmentVariable("NOTIFICATION_CONNECTION")
            ?? "Host=localhost;Database=procureflow_notification;Username=pf;Password=Pf-dev-1234";
