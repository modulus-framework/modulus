...
/// <summary>
/// Composition root for the Approval module. Registers the module's own
...
/// </summary>
public sealed class ApprovalModule : ModulusModule
{
    public override void ConfigureServices(
        IServiceCollection services,
        IConfiguration configuration)
    {
        services.AddModuleDatabase<ApprovalDbContext>(options =>
            options.UseNpgsql(configuration.GetConnectionString("Approval")
                ?? "Host=localhost;Database=procureflow_approval;Username=pf;Password=Pf-dev-1234"));

        services.AddScoped<IUnitOfWork>(sp => sp.GetRequiredService<ApprovalDbContext>());
