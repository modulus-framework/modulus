...
/// <c>modulus migrate</c>) to construct <see cref="ApprovalDbContext"/>
/// without the application's DI container or an HTTP request. At runtime the app
/// still builds the context through DI (see <see cref="ApprovalModule"/>);
/// this type is only used when scaffolding or applying migrations.
/// </summary>
/// <remarks>
/// The connection string is read from the <c>APPROVAL_CONNECTION</c>
...
/// live request state.
/// </remarks>
public sealed class ApprovalDbContextFactory
    : IDesignTimeDbContextFactory<ApprovalDbContext>
{
    public ApprovalDbContext CreateDbContext(string[] args)
    {
        var connectionString =
            Environment.GetEnvironmentVariable("APPROVAL_CONNECTION")
            ?? "Host=localhost;Database=procureflow_approval;Username=pf;Password=Pf-dev-1234";

        var options = new DbContextOptionsBuilder<ApprovalDbContext>()
            .UseNpgsql(connectionString)
            .Options;

        return new ApprovalDbContext(
            options,
