...
public sealed class CodeFlowTemplateTests
{
    ...
    {
        NewAppCommand.WithSignInPage(AppKind.Web, "openiddict", ["users"]).Should().Equal("identity", "users");
        NewAppCommand.WithSignInPage(AppKind.Web, "OpenIddict", []).Should().Equal("identity");
    }

    [Fact]
    public void The_identity_ui_is_not_added_twice()
    {
        NewAppCommand.WithSignInPage(AppKind.Web, "openiddict", ["Identity", "users"]).Should().Equal("Identity", "users");
    }
    ...
    {
        NewAppCommand.WithSignInPage(web ? AppKind.Web : AppKind.Api, auth, ["users"]).Should().Equal("users");
    }
    ...
    {
        App(AppKind.Web).UseCodeFlow.Should().BeTrue();
        App(AppKind.Api).UseCodeFlow.Should().BeFalse("an api app has no login page");
        App(AppKind.Web, auth: "none").UseCodeFlow.Should().BeFalse();
    }

    [Fact]
    public void The_base_settings_turn_the_flow_on_and_leave_the_password_grant_off()
    {
        var settings = _engine.Render("app/appsettings.json", App(AppKind.Web));
    ...
    {
        foreach (var kind in new[] { AppKind.Web, AppKind.Api })
        {
    ...
    {
        var seeding = _engine.Render("identity/IdentitySeeding", App(AppKind.Web));
